...
if [[ $(command -v apt-get) || $(command -v yum) ]] && [[ $(command -v systemctl) ]]; then
	...
	echo -e " 备注: 仅支持 Ubuntu 16+ / Debian 8+ / CentOS 7+ 系统
	" && exit 1
...
v2ray_client_config="/etc/v2ray/fighting86_v2ray_config.json"
backup="/etc/v2ray/fighting86_v2ray_backup.conf"
...
tls_config() {
	...
	while :; do
		...
		read -p "(例如：fighting86.com): " domain
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta} 用来分流的路径 $none , 例如 /fighting86 , 那么只需要输入 fighting86 即可"
		read -p "$(echo -e "(默认: [${cyan}fighting86$none]):")" path
		[[ -z $path ]] && path="fighting86"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}fighting86.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="fighting86.com"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 fighting86.com 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/fighting86/v2ray
		cp -rf $(pwd)/* /etc/v2ray/fighting86/v2ray
	else
		pushd /tmp
		git clone https://github.com/fighting86/v2ray -b "$_gitbranch" /etc/v2ray/fighting86/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/fighting86/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/fighting86/v2ray/config/backup.conf $backup
	cp -f /etc/v2ray/fighting86/v2ray/v2ray.sh $_v2ray_sh
	chmod +x $_v2ray_sh
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=fighting86.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] || $v2ray_transport == 33 ]] && sed -i "36s/=fighting86.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=fighting86/=$path/" $backup
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/fighting86/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/fighting86_v2ray_backup.txt && -d /etc/v2ray/fighting86/v2ray ]]; then
		echo
